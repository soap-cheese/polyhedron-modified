...
class TestVoid(unittest.TestCase):
    ...
    def test_angle1(self):
        s = Edge(R3(0.0, 0.0, 0.0), R3(0.0, 0.0, 1.0))
        self.assertEqual(s.angle, 0)

    def test_angle2(self):
        s = Edge(R3(0.0, 0.0, 0.0), R3(0.0, 1.0, 1.0))
        self.assertAlmostEqual(s.angle, pi/4)

    def test_angle3(self):
        s = Edge(R3(0.0, 0.0, 0.0), R3(0.0, 1.0, 0.0))
        self.assertAlmostEqual(s.angle, pi/2)

    def test_angle4(self):
        s = Edge(R3(0.0, 0.0, 0.0), R3(-1.0, 0.0, -1.0))
        self.assertAlmostEqual(s.angle, pi/4)

    def test_sum_invis1(self):
        s = Edge(R3(0.0, 0.0, 0.0), R3(2.0, 0.0, 0.0))
        self.assertEqual(s.sum_invis(), 0)

    def test_sum_invis2(self):
        s = Edge(R3(0.0, 0.0, 0.0), R3(2.0, 0.0, 0.0))
        f = Facet([R3(-1.0, -1.0, 1.0), R3(-1.0, 1.0, 1.0),
                  R3(3.0, 1.0, 1.0), R3(3.0, -1.0, 1.0)])
        s.shadow(f)
        self.assertEqual(s.sum_invis(), 2.0)

    def test_sum_invis3(self):
        s = Edge(R3(0.0, 0.0, 0.0), R3(2.0, 0.0, 0.0))
        f = Facet([R3(1.0, -1.0, 1.0), R3(1.0, 1.0, 1.0),
                  R3(3.0, 1.0, 1.0), R3(3.0, -1.0, 1.0)])
        s.shadow(f)
        self.assertEqual(s.sum_invis(), 1.0)

    def test_sum_invis4(self):
        s = Edge(R3(0.0, 0.0, 0.0), R3(2.0, 0.0, 0.0))
        f = Facet([R3(0.5, -1.0, 1.0), R3(0.5, 1.0, 1.0),
                  R3(1.5, 1.0, 1.0), R3(1.5, -1.0, 1.0)])
        s.shadow(f)
        self.assertEqual(s.sum_invis(), 1.0)
