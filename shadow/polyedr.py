...
class Edge:
    ...
    def __init__(self, beg, fin):
        ...
        # Угол с вертикалью
        v = fin - beg
        self.angle = acos(abs(v.z / v.len))
        # Длина
        self.len = v.len
        # Центр
        self.center = (beg + fin) * 0.5
    ...
    # Сумма длин невидимых частей
    def sum_invis(self):
        sum_gaps = sum([(self.r3(x.fin) - self.r3(x.beg)).len
                        for x in self.gaps])
        return self.len - sum_gaps
    ...
    def value(self):
        ...
        for e in self.edges:
            for f in self.facets:
                e.shadow(f)
            # Частично видимые
            if not e.gaps or e.gaps[0] == Segment(0, 1):
                continue
            # Угол с вертикалью
            if e.angle > pi/18:
                continue
            # Центр вне куба
            if abs(e.center.x) <= 0.5*self.c and\
               abs(e.center.y) <= 0.5*self.c and\
               abs(e.center.z) <= 0.5*self.c:
                continue
            res += e.sum_invis()
